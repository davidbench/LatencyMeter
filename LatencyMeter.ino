...
NexRadio opt_VA = NexRadio(0, 12, "opt_VA");
NexRadio opt_AV = NexRadio(0, 13, "opt_AV");
...
void Pixel( void * pvParameters ){
  while (true) {
    ...
    micros_now = micros();
    int measured_pixel = micros_now - last_pixel;
...
void Reader( void * pvParameters ){
     while(true){
        ...
        micros_now = (micros() + micros_now)/2;

        refractory = refractory && ((micros_now - result_time)<ref_period);
        
        video_on = PD_val>th1_value;
        if (video_on)
          last_video_on = micros_now;
        
        if (mode_VA) {
          if (video_on && !triggered && !refractory) {
            triggered = true;
            trigger_time = micros_now;
          }
          else {
            triggered = triggered && ((micros_now - trigger_time)<timeout);
          }
        }
        else {
          if (video_on) {
            if (triggered) {
              result = micros_now-trigger_time;
              result_time = micros();
              triggered = false;
              refractory = true;
            }
    ...
    PD_val_pixel        =    0 * video_pixel_just_written + !video_pixel_just_written * max(PD_val_pixel, PD_val);
    Audio_PD_val_pixel  = 4095 * audio_pixel_just_written + !video_pixel_just_written * min(Audio_PD_val_pixel, Audio_PD_val);
    Audio1_val_pixel    = 4095 * audio_pixel_just_written + !video_pixel_just_written * min(Audio1_val_pixel, Audio1_val);
    Audio2_val_pixel    = 4095 * audio_pixel_just_written + !video_pixel_just_written * min(Audio2_val_pixel, Audio2_val);
